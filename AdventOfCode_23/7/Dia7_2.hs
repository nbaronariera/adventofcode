...
-- ## Imports                                  ##
-- ##############################################
   import Data.List (sort, group, elemIndex, sortBy, sortOn)
...
--
   ...
   valores :: [Char]
   valores = ['A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2', 'J']
...
-- ##############################################
   ...
   showl :: [(Naipe, Int)] -> String
   showl ((Naipe n, i):xs) = trace (show n ++ ",") "" ++ (showl xs)
   showl [] = trace("") ""
...
-- ##############################################
   ...
   countSame :: Naipe -> [Int]
   countSame (Naipe naipe) = 
     let js = length (filter (== 'J') naipe)
         sinj = filter (/= 'J') naipe
         resultados = sortOn (\ x -> elemIndex x valores) sinj
     in trace(show $ sumarAlPrimero js resultados) trace(show naipe) [5]

   -- Función para sumar al primero
   sumarAlPrimero :: Int -> [Char] -> [Char]
   sumarAlPrimero _ [] = "JJJJJ" 
   sumarAlPrimero veces lista = (take (1 + veces) (repeat $ head lista)) ++ (tail lista)
